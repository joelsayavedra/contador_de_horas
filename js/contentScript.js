...
fetch(`${window.location.href}&xml=T`)
  ...
  .then(
    ...
    data => {
      ...
      info.hourstotal0 = xmlDoc.getElementsByTagName("hourstotal0")?.[0]?.textContent;
      ...
      for (let i = 0; i < lineasXML.length; i++) {
        ...
        info.lineas.push({
          ...
          hours0: linea.getElementsByTagName('hours0')?.[0]?.textContent,
          ...
          memo0: linea.getElementsByTagName('memo0')?.[0]?.textContent,
